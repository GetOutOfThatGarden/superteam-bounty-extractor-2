...
class PrizeExtractionTester:
    def __init__(self):
        self.results = []
        self.links_file = '/Users/user2/Developer/superteam-bounty-extractor/data/bounty_links.txt'
        self.output_file = 'temp_prize_amounts.json'
    
    def load_bounty_links(self, limit=10):
        """Load first 10 bounty URLs from text file"""
        try:
            with open(self.links_file, 'r', encoding='utf-8') as f:
                links = [line.strip() for line in f if line.strip()][:limit]
            return links
    ...
    async def test_single_bounty(self, page, url):
        """Test prize extraction for a single bounty"""
        ...
        try:
            ...
            # Extract prize breakdown (this will handle View More buttons)
            ...
            result['amounts_match'] = (total_reward == individual_sum) if total_reward else False
            ...
            if prize_breakdown['individual_prizes']:
                prizes_str = ', '.join([f"{p['position']}: {p['amount']}" for p in prize_breakdown['individual_prizes']])
                print(f"  ✓ Prizes: {prizes_str}")
    ...
    async def run_test(self):
        """Run the prize extraction test on first 10 bounties"""
        links = self.load_bounty_links(10)
        if not links:
            print("No bounty links found")
            return
        
        print(f"Testing prize extraction on {len(links)} bounties...")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for i, url in enumerate(links, 1):
                    print(f"\n[{i}/{len(links)}] Testing: {url}")
                    result = await self.test_single_bounty(page, url)
    ...
    def save_results(self):
        ...
        output_data = {
            'test_summary': {
                ...
                'successful_extractions': len([r for r in self.results if r.get('total_reward') is not None]),
                'amount_matches': len([r for r in self.results if r.get('amounts_match', False)]),
                'individual_prizes_found': len([r for r in self.results if r['prize_breakdown']['total_prizes'] > 0])
            },
    ...
    def print_summary(self):
        ...
        successful = len([r for r in self.results if r.get('total_reward') is not None])
        matches = len([r for r in self.results if r.get('amounts_match', False)])
        individual_found = len([r for r in self.results if r['prize_breakdown']['total_prizes'] > 0])
        
        print(f"\n📊 Test Summary:")
        print(f"  Total bounties tested: {total}")
        print(f"  Total rewards extracted: {successful}/{total}")
        print(f"  Individual prizes found: {individual_found}/{total}")
        print(f"  Amount matches: {matches}/{total}")
        
        if individual_found > 0:
            print(f"\n🏆 Bounties with individual prizes:")
            for result in self.results:
                if result['prize_breakdown']['total_prizes'] > 0:
                    prizes = result['prize_breakdown']['individual_prizes']
                    prize_str = ', '.join([f"{p['position']}: {p['amount']}" for p in prizes])
                    print(f"  {result['slug']}: {prize_str}")

# Run the test
async def main():
    tester = PrizeExtractionTester()
